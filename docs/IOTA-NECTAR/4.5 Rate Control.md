...
* *Node identity*. We require node accountability where each message is associated with the node ID of its issuing node (see [Section 3.3 - Peer Discovery](./3.3%20Peer%20Discovery.md)).

* *Timestamp*. In order to verify if the APoW has been performed correctly, message timestamps must pass some quality checks, see [Section 4.2.3 - General Timestamps Rules](./4.2%20Timestamps.md).
